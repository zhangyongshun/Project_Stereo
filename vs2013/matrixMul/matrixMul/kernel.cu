...
cudaError_t matirxMulWithCuda(float **matrix_a, float **matrix_b)
{
	
}
__global__ void matrixMul(float *M, float *N, float *P, int width_a, int width_b)
{
	...
	{
		shared_m[threads_y][threads_x] = M[row * width + (i * TILE_WIDTH) + threads_x];
		shared_n[threads_y][threads_x] = N[column +  + width *((i * TILE_WIDTH) + threads_y)];
		__syncthreads();
