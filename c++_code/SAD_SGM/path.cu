...
__device__ void calculateL(const unsigned int *pre_cost,
	int gradient_intensity, unsigned int *curr_cost,
	const int width, const int height, const int disparity_range, unsigned int max_int)
{
	int min_cost = max_int;
	int d = threadIdx.x;

	int pre_min_cost = max_int;
	for (int pre_d = 0; pre_d < disparity_range; pre_d++)
	{
		...
		if (pre_d - d == 0) {
			...
			pre_min_cost = MIN(pre_min_cost, pre_cost[pre_d] + MAX(P1, gradient_intensity ? P2 / gradient_intensity : P2));
		}
	}
	...
	const int width, const int height, const int disparity_range)
{
	...
	if (col_height < width)
	{
		...
		{
			dev_one_path_cost[(index + col_height + (index)*width)*disparity_range + d] = cost[(index + col_height + (index)*width)*disparity_range + d];
			if (index)
				...
				&dev_one_path_cost[(index + col_height + index*width)*disparity_range], width, height, disparity_range, max);
			__syncthreads();
		}
	}
	else
	{
		...
		{
			dev_one_path_cost[(index + (index + col_height)*width)*disparity_range + d] = cost[(index + (index + col_height)*width)*disparity_range + d];
			...
			if (index)
				...
				&dev_one_path_cost[(index + (index + col_height)*width)*disparity_range], width, height, disparity_range, max);
			__syncthreads();
		}
	}
}
	...
	const int width, const int height, const int disparity_range)
{
	...
	{
		dev_one_path_cost[(col + row*width)*disparity_range + d] = cost[(col + row*width)*disparity_range + d];
		...
		if (row)
			...
			&dev_one_path_cost[(col + row*width)*disparity_range], col, row - 1, disparity_range, max);
		__syncthreads();
	}
}
	...
	const int width, const int height, const int disparity_range)
{
	...
	if (col_height < width)
	{
		...
		{
			dev_one_path_cost[(col_height - index + (index)*width)*disparity_range + d] = cost[(col_height - index + (index)*width)*disparity_range + d];
		//	if (col_height - index - d >= 0)dev_one_path_cost[(col_height - index + (index)*width)*disparity_range + d] = hlr[left_image[(index)*width + (col_height - index)] * 256 + right_image[(index)*width + ((col_height - index) - d)]] - hl[left_image[(index)*width + (col_height - index)]] - hr[right_image[(index)*width + (col_height - index) - d]];
			if (index)
				...
				&dev_one_path_cost[(col_height - index + index*width)*disparity_range], width, height, disparity_range, max);
			__syncthreads();
		}
	}
	else
	{
		...
		{
			dev_one_path_cost[(width - 1 - index + (index + col_height)*width)*disparity_range + d] = cost[(width - 1 - index + (index + col_height)*width)*disparity_range + d];
	...
	const int width, const int height, const int disparity_range)
{
	...
	for (int col = 0; col < width; col++)
	{
		...
		if (col)
			...
			&dev_one_path_cost[(col + row*width)*disparity_range], col - 1, row, disparity_range, max);
		__syncthreads();
	}
}
	...
	const int width, const int height, const int disparity_range)
{
	...
	if (col_height < width)
	{
		...
		{
			dev_one_path_cost[(col_height + index + (height - 1 - index)*width)*disparity_range + d] = cost[(col_height + index + (height - 1 - index)*width)*disparity_range + d];
			...
			if (index)
				...
				&dev_one_path_cost[(col_height + index + (height - 1 - index)*width)*disparity_range], width, height, disparity_range, max);
			__syncthreads();
		}
	}
	else
	...
	const int width, const int height, const int disparity_range)
{
	...
	else
	{
		...
		{
			dev_one_path_cost[(width - 1 - index + (col_height - index)*width)*disparity_range + d] = cost[(width - 1 - index + (col_height - index)*width)*disparity_range + d];
			...
			if (index)
				...
				&dev_one_path_cost[(width - 1 - index + (col_height - index)*width)*disparity_range], width, height, disparity_range, max);
			__syncthreads();
		}
	}
}